...
class FileUpload extends Component {
  ...
  csvToJson = (csv) => {
    ...
    const headers = ["Date", "GPT4", "Gemini", "PaLM2", "Claude", "LLaMA31"]; // Split first row to get headers
    const result = [];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (Object.keys(obj).length && lines[i].trim()) {
        ...
        const parsedObj = {
          ...
          GPT4: parseInt(obj.GPT4),
          Gemini: parseInt(obj.Gemini),
          PaLM2: parseInt(obj.PaLM2),
          Claude: parseInt(obj.Claude),
          LLaMA31: parseInt(obj.LLaMA31),
        };
